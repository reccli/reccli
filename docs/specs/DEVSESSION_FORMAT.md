...
| `start_message_id` / `end_message_id` | string | Yes | First and last messages in the span |
| `start_index` / `end_index` | number | Yes | Exact array bounds using `[start_index, end_index)` semantics |
