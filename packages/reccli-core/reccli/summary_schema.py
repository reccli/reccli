...
def build_message_ids_from_range(start_index: int, end_index: int) -> List[str]:
    """Expand an inclusive-exclusive message range into explicit message IDs."""
    return [f"msg_{idx + 1:03d}" for idx in range(start_index, end_index)]
...
def create_span(
    ...
    if not resolved_message_ids:
        if status == "closed" and isinstance(resolved_end_index, int):
            resolved_message_ids = build_message_ids_from_range(start_index, resolved_end_index)
        elif status == "open" and isinstance(resolved_latest_index, int):
            resolved_message_ids = build_message_ids_from_range(start_index, resolved_latest_index + 1)
...
def ensure_summary_span_links(
    ...
    for category, item in _iter_summary_items(summary):
        ...
        synthesized_span = create_span(
            ...
            end_index=end_index,
            message_ids=build_message_ids_from_range(start_index, end_index),
            topic=topic,
