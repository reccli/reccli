#!/usr/bin/env python3
"""
reccli - One-click CLI recorder with subscription management
The complete version with floating GUI and license checking
"""
...
class ReccliConfig:
    ...
    def start_trial(self):
        ...
        self.config['trial_started'] = now.isoformat()
        self.config['trial_ends'] = (now + datetime.timedelta(days=TRIAL_DAYS)).isoformat()
        self.save_config()
        
        # Open checkout page
        checkout_url = f"https://reccli.com/checkout?device={self.config['device_id']}"
    ...
    def is_licensed(self) -> bool:
        ...
        # During trial
        if self.config.get('trial_ends'):
            trial_end = datetime.datetime.fromisoformat(self.config['trial_ends'])
            if datetime.datetime.now() < trial_end:
                return True
        
        # Active subscription
        if self.config.get('subscription_active'):
            return True
        
        # Check with server (with caching)
        if self.config.get('license_key'):
            # TODO: Implement server check
            return self.config.get('subscription_active', False)
...
class ReccliGUI:
    ...
    def __init__(self):
        ...
        self.menu.add_command(label="💳 Subscription", command=self.open_subscription)
        self.menu.add_separator()
    ...
    def show_trial_dialog(self):
        """Show trial/subscription dialog"""
        dialog = tk.Toplevel(self.root)
        dialog.title("reccli - Start Free Trial")
        ...
        features = [
            ...
            "✓ One-click recording",
            "✓ Perfect playback",
            "✓ Share recordings (soon)",
            "✓ Cloud backup (soon)"
        ]
        
        for feature in features:
            tk.Label(dialog, text=feature, font=('Arial', 11)).pack(anchor='w', padx=50)
        ...
        tk.Button(
            ...
            command=lambda: [self.config.start_trial(), dialog.destroy()],
            bg='#27ae60',
            fg='white',
            font=('Arial', 12, 'bold'),
            padx=20,
...
First Recording: {stats.get('first_recording', 'Never')[:10]}
Last Recording: {stats.get('last_recording', 'Never')[:10]}
...
Share your stats on X!"""
    ...
    def open_subscription(self):
        ...
        device_id = self.config.config.get('device_id', '')
        url = f"https://reccli.com/account?device={device_id}"
        webbrowser.open(url)
...
def main():
    ...
    parser.add_argument('command', nargs='?', default='gui',
                       choices=['gui', 'start', 'stop', 'status', 'activate'],
                       help='Command to execute')
    parser.add_argument('--license', help='License key for activation')
    ...
    if args.command == 'gui':
        ...
        if config.is_licensed():
            ...
            print("   Start your free trial: reccli activate")
    
    elif args.command == 'activate':
        config = ReccliConfig()
        if args.license:
            config.config['license_key'] = args.license
            config.save_config()
            print("✅ License activated!")
        else:
            print("Starting 7-day free trial...")
            config.start_trial()
            print("✅ Check your browser to complete signup")
